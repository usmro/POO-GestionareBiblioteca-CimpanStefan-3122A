...
int main() {
    ...
    tab_comps.push_back(startScreen.Get()); // 0
    tab_comps.push_back(loginScreen.Get()); // 1
    tab_comps.push_back(guestScreen.Get()); // 2
    tab_comps.push_back(userScreen.Get());  // 3
    tab_comps.push_back(biblioScreen.Get()); // 4
    tab_comps.push_back(adminScreen.Get());  // 5
    ...
    auto renderer = Renderer(aplicatie_completa, [&] {
        ...
        else if (state.ecran_activ == 2) ui_content = guestScreen.Render();
        else if (state.ecran_activ == 3) ui_content = userScreen.Render();
        else if (state.ecran_activ == 4) ui_content = biblioScreen.Render();
        else if (state.ecran_activ == 5) ui_content = adminScreen.Render();
